  ...
  <div class="w-full overflow-x-auto bg-white rounded-lg p-2 md:p-6">
    <div class="flex items-center border-b pb-2 justify-between">
      <div class="flex space-x-2 items-center">
        ...
        <p class="text-2xl font-medium text-gray-600">Edit Parent-Dokumen</p>
      </div>
    ...
    <div class="w-full">
      <form action="{{ route('admin.parentdocument.update', $documentParent->id) }}" method="post" class="space-y-2">
        @csrf
        @method('PATCH')
        <div class="space-y-1">
          <label for="" class="text-sm font-medium">Nama</label>
          <input name="name" type="text" value="{{ $documentParent->name }}"
        ...
        <div class="space-y-1">
          ...
          <input name="description" type="text" value="{{ $documentParent->description }}"
            class="w-full px-2 py-2 rounded ring-gray-300 focus:ring-2 focus:ring-blue-400 outline-none ring-1">
        </div>
...
<div class="w-full md:w-1/3 lg:w-1/4 h-auto bg-white rounded-lg p-6">
  ...
  <form action="{{ route('admin.parentdocument.destroy', $documentParent->id) }}" method="post"
